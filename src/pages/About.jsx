...
export default function About({ videoBackground = false, onVideoReady }) {
  ...
  useEffect(() => {
    if (profileLoaded && !videoBackground) {
      setFadeIn(true);
      setTimeout(() => setAboutContentLoaded(true), 1100);
    }
  }, [profileLoaded, setAboutContentLoaded, videoBackground]);
  ...
  return (
    <main className="h-auto top-0 left-0">
      ...
      <div
        id="home"
        className={`w-screen min-h-screen md:h-screen flex flex-col relative overflow-hidden ${!videoBackground ? 'bg-banner bg-cover bg-center' : ''}`}
      >
        {videoBackground && (
          <video
        ...
        {/* Hero Dashboard */}
        <div
          className={`flex flex-col items-center mx-4 md:mx-10 ${!videoBackground ? `transition-opacity duration-1000 ${fadeIn ? "opacity-100" : "opacity-0"}` : ''}`}
        >
          ...
          <div className="relative z-10 w-full h-full flex items-center justify-center px-4 md:px-8 py-4 md:py-6">
            <div className="w-full max-w-7xl min-h-[90vh] md:h-[75vh] grid grid-cols-1 md:grid-cols-12 grid-rows-auto md:grid-rows-6 gap-2 md:gap-3">
              ...
              <div className="col-span-1 md:col-span-4 row-span-1 md:row-span-6 min-h-[88vh] md:min-h-0 bg-black/10 backdrop-blur-md rounded-2xl p-3 md:p-4 shadow-2xl border border-white/20 flex flex-col items-center justify-center text-center">
                <img
                  className="rounded-full border-3 border-white/30 shadow-xl w-24 md:w-40 h-24 md:h-40 object-cover mb-2 md:mb-4"
                  src={profile}
                  alt="profile"
                  onLoad={videoBackground ? undefined : handleProfileLoad}
                />
      ...
      {/* Content sections — no opacity transition when video overlay handles the reveal */}
      <div className={`flex flex-col items-center mx-4 md:mx-10 ${!videoBackground ? `transition-opacity duration-1000 ${fadeIn ? "opacity-100" : "opacity-0"}` : ''}`}>
        <div id="about" className="flex flex-col justify-center h-max w-full md:w-11/12 lg:w-4/5 px-6 md:px-0">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex-1 rounded-2xl bg-white/70 backdrop-blur-md border border-gray-100 shadow-lg p-4 md:p-6 transition-all">
            ...
            <div className="flex-1 rounded-2xl bg-white/70 backdrop-blur-md border border-gray-100 shadow-lg p-4 md:p-6 transition-all">
              ...
              <div className="hidden md:block">
                ...
                <p className="text-sm mb-2">• Master of Engineering Student Ambassador</p>
              </div>
            </div>
          </div>
        </div>
        <div id="experience" className="flex flex-col justify-center h-max w-full md:w-11/12 lg:w-4/5 px-6 md:px-0">
          ...
          {/* Tesla */}
          <div className="rounded-2xl bg-white/70 backdrop-blur-md border border-gray-100 shadow-lg p-4 md:p-6 mb-4 md:mb-6 transition-all">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-0 md:mb-4 gap-3 md:gap-0">
              <div className="flex items-center gap-3">
                <img className="w-20 h-20 object-contain" src="/images/tesla.svg" alt="Tesla" />
                <div>
                  <h2 className="font-bold text-lg">Software QA Engineer</h2>
                  <h3 className="font-semibold text-gray-700">Tesla</h3>
                </div>
              </div>
              <div className="text-left md:text-right">
                <p className="text-sm text-gray-600">Jul 2026 – Present</p>
                <p className="text-sm text-gray-500">Palo Alto, CA</p>
              </div>
            </div>
            <div className="hidden md:block pl-15 max-w-2xl">
              <p className="text-sm mb-2">• Beginning July 2026.</p>
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-white/70 backdrop-blur-md border border-gray-100 shadow-lg p-4 md:p-6 mb-4 md:mb-6 transition-all">
            ...
            <div className="hidden md:block pl-15 max-w-2xl">
              <p className="text-sm mb-2">• UC Berkeley Master of Engineering Capstone Project with Prof. Allen Yang on Robot Open Autonomous Racing (roar.berkeley.edu)</p>
              <p className="text-sm mb-2">• Implementing DLIO++ (Direct LiDAR-Inertial Odometry), a localization system using LiDAR-IMU fusion for autonomous racecars in C++, Python, and ROS 2.</p>
              <a
                href="https://github.com/augcog/DLIO_plusplus/tree/ucb-roar#"
                target="_blank"
                rel="noreferrer"
